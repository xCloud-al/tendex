...
import { UserCheck, Mail, Phone, Award, Star, ArrowLeft, FileText } from 'lucide-react';
...
const EvaluatorEdit: React.FC = () => {
    ...
    if (id) {
      ...
      if (evaluator) {
        setFormData({
          ...
          status: evaluator.status as 'active' | 'inactive',
          rating: evaluator.rating,
          assignedTenders: evaluator.assignedTenders
        });
      }
    }
  }, [id]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    addToast('Evaluator updated successfully!', 'success');
    navigate(`/evaluators/${id}`);
  };
    ...
    <div className="animate-fade-in">
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          >
            ...
            Back to Evaluator Details
          </Link>
          <h1 className="text-3xl font-bold heading-underline mb-2">Edit Evaluator</h1>
          <p className="text-neutral-600">Update evaluator information and expertise</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm border border-neutral-200 p-6">
        ...
        <div className="flex justify-end space-x-4 mt-6">
          <button
            type="button"
            onClick={() => navigate(`/evaluators/${id}`)}
